...
//               product={{
//                 ...product,
//                 slug: product.slug // Pass the entire slug object here
//               }}
...
export default function WishlistPage() {
  const { toast } = useToast()
  // Change 'any[]' to Product[] for proper typing
  ...
  useEffect(() => {
    const fetchWishlistItems = async () => {
      ...
      try {
        ...
        if (response.length > 0) {
          const uniqueProducts = response[0].items.map((item: { product: Product }) => item.product)
          setWishlistItems(uniqueProducts)
        } else {
          setWishlistItems([])
        }
            ...
            <ProductWishlistCard
              ...
              product={{
                ...
                images: product.images.map((image) => ({
                  url: image.asset.url,
                  alt: product.name, // Provide a default alt text
                })),
              }} // Map images to match the expected structure
              onRemove={() => removeFromWishlist(product.slug.current)} // Access slug.current directly
            />
          ))}
        </div>
      ) : (
